...
- **남은 Task 1**: 비-lookup 텍스트 필드(배달일자/시간·배달장소·받는분·핸드폰·경조사어·보내는이·카드내용·
  발주자·휴대폰)와 수량의 폼기준 상대좌표를 안전하게 실측 → `layout.FIELD_POSITIONS` 완성. 상품/고객은
  lookup 이라 좌표+서브루틴으로 별도 처리.
